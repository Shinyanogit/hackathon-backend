...
func (h *ItemHandler) Create(c echo.Context) error {
	...
	if err := c.Bind(&req); err != nil {
		return c.JSON(http.StatusBadRequest, ErrorResponse("bad_request", "invalid json"))
	}
...
func (h *ItemHandler) Get(c echo.Context) error {
	...
	if err != nil {
		return c.JSON(http.StatusBadRequest, ErrorResponse("bad_request", "invalid id"))
	}
	item, err := h.svc.Get(c.Request().Context(), id)
	if err != nil {
		if err == service.ErrNotFound {
			return c.JSON(http.StatusNotFound, ErrorResponse("not_found", "item not found"))
		}
		return c.JSON(http.StatusInternalServerError, ErrorResponse("internal_error", "failed to fetch item"))
	}
...
func (h *ItemHandler) List(c echo.Context) error {
	...
	if err != nil {
		return c.JSON(http.StatusInternalServerError, ErrorResponse("internal_error", "failed to fetch items"))
	}
